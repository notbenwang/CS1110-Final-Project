...
#    __slots__ = ["x","y","speedx","speedy","_w","_h","_key","_image","_color"]
    def __init__(self, x, y, image, color, w=None, h=None, degree=0):
    ...
    def set_angle(self, angle):
        difference = self.__dict__['degree'] - angle
        key = self.__dict__['_key']
        self.__dict__['degree'] = angle
        self._set_key(key[0], key[1], key[2], key[3], key[4] + difference)
...
def timer_loop(fps, callback, limit=None):
    ...
    pygame.display.set_caption('Night of the Poorly Made Zombies')
